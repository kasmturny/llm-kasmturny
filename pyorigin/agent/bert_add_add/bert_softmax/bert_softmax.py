    ...
    def evaluate(self, dev_loader, model, mode='dev'):
        ...
        with torch.no_grad():
            for idx, batch_samples in enumerate(dev_loader):
                ...
                pred_tags.extend(
                    [[id2label.get(idx) for idx in indices] for indices in np.argmax(batch_output, axis=2)])
                true_tags.extend(
                    [[id2label.get(idx) if idx != -1 else 'O' for idx in indices] for indices in batch_tags])
