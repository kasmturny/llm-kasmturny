...
def convert_to_json(origin_path, output_path=None):
    with open(origin_path, 'r', encoding='utf-8') as f:
        ...
        for line in origin_data:
            ...
            这十个实体进行识别，并返回识别结果。例如输入input:"浙商银行企业信贷部叶老桂博士则从另一个角度对五道门槛进行了解读。叶老桂认为，对目前国内商业银行而言，"，那么输出output:"{
            "entities":[{"entity_text": "叶老桂","entity_type": "name"},{"entity_text": "浙商银行","entity_type": 
            "company"}]}"。 1、注意输出一定要是json格式的字符串. 2、entity_text的值必须是原文中的连续的文本，不能随意截取拼接原文中的文本. 
